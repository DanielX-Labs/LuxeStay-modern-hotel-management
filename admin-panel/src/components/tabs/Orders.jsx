...
function Orders() {
  ...
  const invoice = async (row, action = 'view') => { const win = window.open('', '_blank'); if (!win) { notificationWithIcon('error', 'PDF VIEWER BLOCKED', 'Allow pop-ups for this site, then try again.'); return; } try { win.document.title = 'Loading invoice...'; const pdf = await ApiService.get(`/api/v1/booking/${row.id}/invoice?format=pdf`, { responseType: 'blob' }); const blob = pdf instanceof Blob && pdf.type === 'application/pdf' ? pdf : new Blob([pdf], { type: 'application/pdf' }); const url = URL.createObjectURL(blob); win.location.replace(url); if (action === 'print') win.addEventListener('load', () => win.print()); setTimeout(() => URL.revokeObjectURL(url), 300000); notificationWithIcon('success', action === 'print' ? 'INVOICE READY TO PRINT' : 'PDF VIEWER OPENED', action === 'print' ? `${row.booking_id} is ready to print.` : 'Use the Adobe or browser PDF toolbar to download the invoice.'); } catch (err) { win.close(); notificationWithIcon('error', 'INVOICE UNAVAILABLE', err?.response?.data?.result?.error || err?.message || 'The invoice could not be opened.'); } };
  ...
  const columns = [
    ...
    { title: 'Actions', fixed: 'right', width: 250, render: (_, row) => <Space wrap>
      ...
      <Button size='small' onClick={() => invoice(row)} disabled={!row.invoice_available}>Open PDF</Button>
      <Button size='small' onClick={() => invoice(row, 'print')} disabled={!row.invoice_available}>Print</Button>
