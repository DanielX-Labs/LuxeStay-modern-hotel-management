...
function CreateRoom() {
  ...
  const onFinish = (values) => {
    const selectedImages = (values.room_images || fileList)
      .map((image) => image.originFileObj || image)
      .filter((image) => image instanceof File);

    if (!selectedImages.length) {
      notificationWithIcon('error', 'ROOM IMAGES REQUIRED', 'Please select at least one supported image');
      return;
    }
    ...
    for (const image of selectedImages) {
      formdata.append('room_images', image, image.name);
    }
