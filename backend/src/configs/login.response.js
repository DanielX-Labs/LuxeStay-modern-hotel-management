...
const loginResponse = (res, user, maintenance) => {
  ...
  // options for cookie
  const options = {
    expires: new Date(Date.now() + process.env.COOKIE_EXPIRES_DAYS * 24 * 60 * 60 * 1000),
    httpOnly: true
  };
  ...
  res
    ...
    .json({
      ...
      access_token_expires: getDateAfterDuration(process.env.JWT_ACCESS_EXPIRES),
      refresh_token_expires: getDateAfterDuration(process.env.JWT_REFRESH_EXPIRES),
      result: {
        title: 'SUCCESS',
        ...
        data: {
          ...
          email: user.email,
          phone: user.phone,
          avatar: publicAssetUrl(user.avatar),
          gender: user.gender,
          dob: user.dob,
