...
function OrderPlaceModal({ bookingModal, setBookingModal }) {
  ...
  const handlePlacedOrder = () => {
    if (selectedDates.length === 0) {
      notificationWithIcon('error', 'ERROR', 'Select at least one night for your stay.');
    } else if (selectedDates.length > 5) {
      notificationWithIcon('error', 'ERROR', 'You can select a maximum of five nights.');
    } else {
      confirm({
        title: 'Confirm these stay dates?',
